...
public:
    ...
    void withdrawAmount(int accNumber, double withdrawAmount)
    {
        // Search account exists in our array with that account number
        int index = getIndexWithAccNumber(accNumber);
        if (index == -1)
        ...
        else
        {
            ...
            Account existingCustomer = accounts[index];
            // Withdraw
            if (existingCustomer.getBalance() - withdrawAmount >= 0)
            {
                // then only withdraw
                existingCustomer.debit(withdrawAmount);
                accounts[index] = existingCustomer;
            }
            else
            {
                cout << "Insufficient Balance!\nAvailable balance is = " << existingCustomer.getBalance() << endl;
            }
        }
    }
